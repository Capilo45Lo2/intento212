...
import com.exame.intento212.databinding.ActivityForgetPasswordBinding
...
class ForgetPasswordActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding?.btnForgotPasswordSubmit?.setOnClickListener { restPassword() }
    }

    private fun validateForm(email: String):Boolean {
        return when {
            TextUtils.isEmpty(email) && !Patterns.EMAIL_ADDRESS.matcher(email).matches() -> {
                binding?.tilEmailForgetPassword?.error = "ingrese correo"
                false
            }
            ...
            if(validateForm(email)){
                ...
                auth.sendPasswordResetEmail(email).addOnCompleteListener{task->
                    if (task.isSuccessful)
                    {
                        ...
                        hideProgressBar()
                        showToast(this, "No se pudo restaurar")
                    }
                }
            }
